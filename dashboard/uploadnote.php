...
if (isset($_SESSION['id'])) {
    ...
    $validExt = array ('pdf', 'doc', 'docx');
    if (empty($file)) {
echo "<script>alert('Attach a file');</script>";
    }
    else if ($_FILES['file']['size'] <= 0 || $_FILES['file']['size'] > 30720000 )
    {
echo "<script>alert('File size exceeds limits. Please compress and upload again');</script>";
    }
        ...
        if(move_uploaded_file($_FILES['file']['tmp_name'], $folder.$notefile)) {
            ...
            if (mysqli_affected_rows($conn) > 0) {
                echo "<script> alert('File uploaded successfully. It will be queued for printing after an admin approves it');
...
?>
	...
	<div class="form-group">
		...
		<input type="text" name="title" class="form-control" placeholder="Eg: Project, assignment, document"  value = "<?php if(isset($_POST['upload'])) {
            echo $file_title; } ?>" required="">
	</div>

	<div class="form-group">
		<label for="post_tags">Short Description</label>
		<input type="text" name="description" class="form-control" placeholder="Eg: Lab record for Web Programming ...." value="<?php if(isset($_POST['upload'])) {
            echo $file_description;  } ?>" required="" ">
	</div>

	 <div class="form-group">
        <label for="post_image">Select File</label><font color="brown"> (allowed file type: 'pdf','doc', 'docx' | allowed maximum size: 30 mb ) </font>
